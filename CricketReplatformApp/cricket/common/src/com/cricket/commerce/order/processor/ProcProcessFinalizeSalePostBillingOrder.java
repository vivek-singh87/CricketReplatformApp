...
public class ProcProcessFinalizeSalePostBillingOrder extends GenericService implements PipelineProcessor {
	...
	{ 
		//logInfo("[ProcProcessFinalizeSalePostBillingOrder->runProcess()]: Entering into runProcess()...");
        ...
        if (order == null) {
            throw new InvalidParameterException("[ProcProcessFinalizeSalePostBillingOrder->runProcess()]: order Id is not valid");
        }
        CricketOrderImpl cricketOrder = (CricketOrderImpl) order;
        if (isLoggingDebug()) {			 					
	    		logDebug("Inside the ProcProcessFinalizeSalePostBillingOrder class of runProcess() method :::" + CricketCommonConstants.SESSION_ID + getSessionId() + CricketCommonConstants.ORDER_ID + cricketOrder.getId());
		}
        ...
        CricketAccountHolderAddressData  accountHolderAddressData = (CricketAccountHolderAddressData)accountHolderAddressDataObj;
         logInfo("[ProcProcessFinalizeSalePostBillingOrder->runProcess()]: Entering into runProcess()...Order Id:"+cricketOrder.getId()+ " || For the User: "+
        ...
        try{	     
        	if(isLoggingDebug()){
        		 CricketOrderTools ordertools =(CricketOrderTools) getOrderManager().getOrderTools();
   	    		logDebug("[ProcProcessFinalizeSalePostBillingOrder->runProcess()]: Order Details :");
   	    		logDebug(ordertools.orderDetailsToTrack(cricketOrder,null,this.getClass().getName()));
   	    	}
        	starttime = System.currentTimeMillis();
        	FinalizeSaleResponseVO responseVO = finalizeSale(cricketOrder);
        	endtime = System.currentTimeMillis();
            
        	logInfo("[ProcProcessFinalizeSalePostBillingOrder->runProcess()]: time taken to get ESP response in milliseconds:  "+ (endtime - starttime));
            ...
            if(!responseStatus.getCode().equalsIgnoreCase(ESP_SUCCESS)){
            	...
            	logInfo("[ProcProcessFinalizeSalePostBillingOrder->runProcess()]: Exiting runProcess(), with return STOP_CHAIN_EXECUTION...");
            	return PipelineProcessor.STOP_CHAIN_EXECUTION;
            }
	        
	    	logInfo("[ProcProcessFinalizeSalePostBillingOrder->runProcess()]: Exiting runProcess(), with return SUCCESS..." + getCricketESPAdapter().getSessionOrderRequestConversationInfo(cricketOrder));
		    return SUCCESS;
        } 
        
        catch (ServiceException se) {
        	vlogError("[ProcProcessFinalizeSalePostBillingOrder->runProcess()]:" + CricketESPConstants.WHOOP_KEYWORD + "ServiceException while getting response:"+ "ConversationId: " + getCricketESPAdapter().getConversationId() + CricketCommonConstants.SESSION_ID + getSessionId() + CricketCommonConstants.ORDER_ID +  cricketOrder.getId(), se);
			...
			vlogError("[ProcProcessFinalizeSalePostBillingOrder->runProcess()]:" + CricketESPConstants.WHOOP_KEYWORD + " ESPApplicationException while getting response:"+ "ConversationId: " + getCricketESPAdapter().getConversationId() + CricketCommonConstants.SESSION_ID + getSessionId() + CricketCommonConstants.ORDER_ID +  cricketOrder.getId(), eae);
			...
			vlogError("[ProcProcessFinalizeSalePostBillingOrder->runProcess()]:" + CricketESPConstants.WHOOP_KEYWORD +  " RemoteException while getting response:"+ "ConversationId: " + getCricketESPAdapter().getConversationId() + CricketCommonConstants.SESSION_ID + getSessionId() + CricketCommonConstants.ORDER_ID +  cricketOrder.getId(), re);
			...
			vlogError("[ProcProcessFinalizeSalePostBillingOrder->runProcess()]: " + CricketESPConstants.WHOOP_KEYWORD +  " Exception while getting response:"+ "ConversationId: " + getCricketESPAdapter().getConversationId() + CricketCommonConstants.SESSION_ID + getSessionId() + CricketCommonConstants.ORDER_ID +  cricketOrder.getId(), e);
			pResult.addError(CricketESPConstants.ESP_SYSTEM_EXCEPTIONS,CricketESPConstants.ESP_SYSTEM_EXCEPTIONS);
		}
		return PipelineProcessor.STOP_CHAIN_EXECUTION;       
		//return SUCCESS;
	 ...
	 */
	private FinalizeSaleResponseVO finalizeSale(CricketOrderImpl pCricketOrder) throws CricketException, ESPApplicationException, RemoteException, MalformedURLException, ServiceException {
		if (isLoggingDebug()) {
    		logDebug("Inside the ProcProcessFinalizeSalePostBillingOrder class of finalizeSale() method..." + CricketCommonConstants.SESSION_ID + getSessionId() + CricketCommonConstants.ORDER_ID + pCricketOrder.getId());
		}
		FinalizeSaleResponseVO finalizeSaleResponse = getCricketESPAdapter().finalizeSalePostBillingOrder(pCricketOrder);
		if (isLoggingDebug()) {			
    		logDebug("Exiting from ProcProcessFinalizeSalePostBillingOrder class of finalizeSale() method..." + CricketCommonConstants.SESSION_ID + getSessionId() + CricketCommonConstants.ORDER_ID + pCricketOrder.getId());
		}
