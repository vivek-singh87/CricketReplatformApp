				...
				<dsp:param name="filterBySite" value="false"/>
       			<dsp:param name="filterByCatalog" value="false"/>
					...
					<dsp:oparam name="output">
						...
						<c:choose>
							<c:when test="${product.type eq 'plan-product'}">
								<c:set var="plan" value="${plan+1}"/>
							</c:when>
							<c:when test="${product.type eq 'phone-product'}">
								<c:set var="phone" value="${phone+1}"/>
							</c:when>
							<c:when test="${product.type eq 'addOn-product'}">
			...
			<c:otherwise>
				<div class="row">
				  <div class="large-12 small-12 columns no-padding">
				    <div data-alert class="alert-box">
				    	<c:choose>
				    		<c:when test="${phone eq '0' && plan eq '0' }">
