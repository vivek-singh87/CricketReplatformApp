...
<script type="text/javascript">
	...
	function reloadTopPhoneDisplaySection (productId, skuId, skuimage, contextpath, CartButton, isOOFPhone, ProfilemarketType, editPhone) {
	//alert('1');
		var url = contextpath + "/browse/phone/details/includes/displayPhoneNamewithZipcode.jsp?skuId=" + skuId + "&productId=" + productId + "&editPhone=" +editPhone;
		$.get(url, function(data) {
			$('#phoneDisplayTopSection').html(data);
			var image =  skuimage;
			changeProductImageAndPricing(image);
			//changeAddToCartSection(skuId, productId, contextpath,CartButton,isOOFPhone,ProfilemarketType,editPhone);
		});
	}
	...
	function changeAddToCartSection(skuId, productId, contextpath,CartButton,isOOFPhone,ProfilemarketType, editPhone){		
		var url = contextpath + "/browse/phone/details/includes/displayAddToCart.jsp?skuId="+skuId+"&productId="+productId+"&CartButton=" + CartButton+"&isOOFPhone="+isOOFPhone+"&ProfilemarketType="+ProfilemarketType+"&editPhone="+editPhone;			
		$.get(url, function(data) {		
			$('#addToCartSectionOnColorChange').html(data);
		});
	}
      ...
      <div class="row" style="position:relative;">
	   ...
	   <!-- END displaying SKU thumbnail images -->	
		...
		<!-- Dynamically load all SKUs for this phone; identify here by a colour but in real use would just be a product sku number-->
			...
			<dsp:getvalueof var="isOOFPhone" value="${product.isOOFPhone}"/>
			<c:choose>
				<c:when test="${(ProfilemarketType eq 'OOF') and (isOOFPhone eq false or isOOFPhone eq null)}">				
					<dsp:getvalueof var="CartButton" value="disable"/>
				</c:when>
				<c:otherwise>
					<dsp:getvalueof var="CartButton" value="enable"/>
				</c:otherwise>
			</c:choose>				
			<dsp:droplet name="/atg/dynamo/droplet/ForEach">
				<dsp:param name="array" value="${product.childSKUs}"/>
								...
								<c:choose>
									...
									<c:otherwise>	
										<dsp:getvalueof var="editPhone" param="editPhone"/>
											<a href="javascript:reloadTopPhoneDisplaySection('${productIdForReload}','${skuId}','${skuLargeImage}','${contextpath}','enable','${isOOFPhone}','3G', '${editPhone}')" id="${swatchName}" class="color-swatch1">
												<span>	
													<!-- START Liquid Pixel Image display droplet -->
														<dsp:droplet name="/com/cricket/imageurl/ImageURLLookupDroplet">
         ...
         <dsp:getvalueof var="manuallyEnteredZipCode" bean="CitySessionInfoObject.cityVO.manulaEntry"/>	
           <c:choose>
           		<c:when test="${manuallyEnteredZipCode eq false}">	
	           		<div class="prices">
			            ...
			            <div data-alert class="alert-box show-for-small">
			           		<crs:outMessage key="cricket_phonedetails_Pleaseenteryour"/><a href="#"><crs:outMessage key="cricket_phonedetails_zipcode"/></a><crs:outMessage key="cricket_phonedetails_toseepricingandaddtoyourcart"/> 
			            </div>
				    </div>
				...
				<c:otherwise>
					<!-- This section is to show Price, promotion and final price based on the color / Sku selected -->
					<dsp:droplet name="/atg/commerce/catalog/SKULookup">
						<dsp:param name="id" param="skuId"/>
						<dsp:param name="elementName" value="sku"/>	
						<dsp:param name="filterBySite" value="false"/>
					...
					<div class="text-right right" id="addToCartSectionOnColorChange">
						<dsp:include page="/browse/phone/details/includes/displayAddToCart.jsp">
							<dsp:param name="skuId" param="skuId"/>
							<dsp:param name="productId" value="${product.id}"/>
							<dsp:param name="CartButton" value="${CartButton}"/>
							<dsp:param name="ProfilemarketType" value="${ProfilemarketType}"/>
							<dsp:param name="isOOFPhone" value="${isOOFPhone}"/>
						</dsp:include>
					</div>
				</c:otherwise>
			</c:choose>
