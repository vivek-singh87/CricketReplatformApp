  ...
  <!-- Desktop layout for Overview -->
                ...
                <thead>
                  <tr>
					...
					<dsp:include page="/checkout/payment/includes/display_totalOrderAmount.jsp">
					...
					<dsp:getvalueof var="workOrderType" bean='ShoppingCart.current.workOrderType' />
					 ...
					 <div class="incentive" id="autoBillPaymentText">
   						<c:choose>
							<c:when	test="${(networkProvider eq 'CRICKET' && workOrderType ne 'OXC' && workOrderType ne 'RRC')}">
								<p><crs:outMessage key="cricket_checkout_$5_discount_message"/><!-- This includes the $5 discount for automatic bill pay. --></p>
							</c:when>
							<c:when	test="${(ProfilemarketType ne 'OOF' && customerType ne 'O' && workOrderType ne 'OXC' && workOrderType ne 'RRC')}">
								<p><crs:outMessage key="cricket_checkout_$5_discount_message"/><!-- This includes the $5 discount for automatic bill pay. --></p>
 							</c:when>
						</c:choose>	
					</div>
					<div>
						<a href="/checkout/confirm/confirmOrder.jsp" class="button small btn-checkout-step-2"><span><crs:outMessage key="cricket_checkout_next"/><!-- Next --></span>- <crs:outMessage key="cricket_checkout_confirm_order"/><!--   - Confirm Order --></a>
					</div>
				</div>
			...
			<section id="overview-total">
	      	<div class="row show-for-small summary">
					...
					<div class="incentive" id="autoBillPaymentTextMobile">					
					<c:choose>
						<c:when	test="${(networkProvider eq 'CRICKET' && workOrderType ne 'OXC' && workOrderType ne 'RRC')}">
							<p><crs:outMessage key="cricket_checkout_$5_discount_message"/><!-- This includes the $5 discount for automatic bill pay. --></p>
						</c:when>
